...
export const CertificateInfo = ({ cardInfo, isOpen }: ICertificateInfo) => {
  ...
  return (
    ...
    >
      <h3 className={`text-primary text-center font-bold text-base line-clamp-2`}>
        {cardInfo.title}
      </h3>
      ...
      <a href={cardInfo.certificateFile.url} target="_blank" className="text-white text-center mx-auto w-fit px-4 block rounded-lg border-2 focus:ring-2 focus:ring-secondary border-primary duration-300 bg-panel hover:scale-105 hover:shadow-primary hover:shadow-allAround hover:border-white hover:bg-primary"><FontAwesomeIcon icon={faEye}/> Visualizar Certificado</a>
    </div>
  );
};
